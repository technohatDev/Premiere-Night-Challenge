...
import { getPosterUrl, getBackdropUrl } from "@/utils/imageUrl";
import { getYear, formatRuntime } from "@/utils/formatDate";
...
export default function MovieDetailScreen() {
  ...
  if (isError || !movie) {
    return <ErrorState message="Could not load movie details" onRetry={refetch} />;
  }
  ...
  return (
    <View className="flex-1 bg-background">
      <ScrollView contentContainerStyle={{ paddingBottom: 32 }}>
        ...
        {/* Tagline */}
        {movie.tagline ? (
          <Text className="mt-4 px-4 text-sm italic text-text-muted">
            "{movie.tagline}"
          </Text>
        ) : null}
